...
describe('Ast generator (with a single *.langium file in the project only)', () => {
    ...
    // TODO warum sollen called Terminals nicht uncludiert werden?
...
describe('Ast generator (with multiple *.langium files)', () => {
    ...
    test('Multi-file project: Grammar "one" uses a type which is inferred in file "two", but rule in "two" is not reachable', async () => {
    ...
    test('Multi-file project: Same name "test" for project, file name and language id (works even with grammar file names which produce non-unique identifiers)', async () => {
        await testMultiProject(
            [{
                path: 'test.langium',
                languageID: 'test',
...
async function testMultiProject(grammarFiles: GrammarFile[], moreExpectedParts: string[]): Promise<void> {
    ...
    if (configMap.size === 1) {
        ...
        for (const e of [...configMap.entries()]) {
            ...
            const embeddedGrammar = await embedGrammars([entryGrammar], configMap, config, {}, services.grammar);
