...
export class LangiumParser extends AbstractLangiumParser {
    ...
    consume(idx: number, tokenType: TokenType, feature: AbstractElement): void {
        ...
        if (!this.isRecording()) {
            ...
            const { assignment, targetType } = this.getAssignment(feature);
            const current = this.current;
            if (assignment) {
                const convertedValue = isKeyword(feature) ? token.image : this.converter.convert(token.image, leafNode);
                this.assign(assignment.operator, assignment.feature, convertedValue, leafNode, targetType);
    ...
    private performSubruleAssignment(result: any, feature: AbstractElement, cstNode: CompositeCstNode | undefined): void {
        ...
        if (assignment && cstNode) {
            this.assign(assignment.operator, assignment.feature, result, cstNode, targetType);
        } else if (!assignment) {
    ...
    private getAssignment(feature: AbstractElement): AssignmentElement {
        if (!this.assignmentMap.has(feature)) {
            ...
            if (assignment && isCrossReference(assignment.terminal) && assignment.terminal.type.ref) {
                const type = assignment.terminal.type.ref;
                if (type) {
                    targetType = getTypeName(type);
                }
            }
    ...
    private assign(operator: string, feature: string, value: unknown, cstNode: CstNode, targetType?: string): void {
        const obj = this.current;
        let item: unknown;
        if (targetType && typeof value === 'string') {
            item = this.linker.buildReference({
                node: obj,
                property: feature,
                refNode: cstNode,
                refText: value,
