...
describe('Infix operator parsing', async () => {
    ...
    test('Should work with different infix rules that return the same type #1', async () => {
        const grammar = `
            ...
            infix BinaryExpr on BinaryExpr2 returns BinaryExpression:
                '*' | '/';
        ...
        const services = await createServicesForGrammar({ grammar });
        const parse = parseHelper(services);
        const document = await parse('1 + 2 * 3');
        expect(document.parseResult.parserErrors).toHaveLength(0);
        const node = (document.parseResult.value as ExprModel).expr;
        expect(node.$type).toBe('BinaryExpression');
...
describe('AST feature values', () => {
    test('Should create a string for a feature with a string data rule containing a single terminal', async () => {
        ...
        `;
        const services = await createServicesForGrammar({ grammar: text });
        const result = services.parser.LangiumParser.parse('3 9');
        expect(result.parserErrors).toHaveLength(0);
        expect(result.value).toHaveProperty('name', '3');
    ...
    test('Should create a string for a feature with a string data rule containing multiple terminals', async () => {
        const text = `
            grammar Test
            entry A: 'name' name=B 'b' b=B;
            B returns string: INT '.' INT;
            terminal INT returns number: /[0-9]+/;
            hidden terminal WS: /\\s+/;
        `;
        const services = await createServicesForGrammar({ grammar: text });
        const result = services.parser.LangiumParser.parse('name 3.1 b 9.63');
        expect(result.parserErrors).toHaveLength(0);
        expect(result.value).toHaveProperty('name', '3.1');
    ...
    test('Should create a number for a feature with a number data rule containing multiple terminals', async () => {
        ...
        `;
        const services = await createServicesForGrammar({ grammar: text });
        const result = services.parser.LangiumParser.parse('0x7 0x20');
        expect(result.parserErrors).toHaveLength(0);
        expect(result.value).toHaveProperty('name', 7);
