...
class LangiumGenerator extends Generator
{
    ...
    _ifLanguageIdCorrect(): boolean {
          // FIX: is it nice identifier?
        return new RegExp('^[a-zA-Z0-9\-]+$').test(this.answers.language_id.toString());          
    }
    ...
    // FIX: types
    _replaceInTemplate(answers: any, content: any): string {
    ...
    writing(): void {
        if (!this._ifLanguageIdCorrect()) {
          this.log("A language identifier can be NOT CORRECT ID");
        } else {
          this.fs.copy(
