...
class LangiumGenerator extends Generator
{
    ...
    async prompting(): Promise<void> {
        this.answers = await this.prompt([ 
          ...
          },
          {
            ...
            validate: (input: string): boolean | string =>
              /^[a-z]*$/.test(input) ? true : "Extension can contain only small letters. Try again."
          }
        ]);
    }
    ...
    writing(): void {
        ...
        [".", ".vscode", ".eslintrc.json", ".vscodeignore"].map(path => {
          const replaceTemplateWords = (answers: any, content: Buffer): string =>
          ...
          const replaceTemplateNames = (answers: any, path: string): string =>
            path
